...
phases presents a potential challenge for this replication attempt.
Screening participants for attention before letting them begin the study
is one way of ensuring that participants follow the instructions rather
than passively view the stimuli. Including more detailed written
instructions for implementing distraction and reappraisal during the
training and practice phases will also help maximize correct
...
images (2 low-intensity and 2 high-intensity) while implementing one of
two regulatory strategies: </br> </br> Distraction: thinking about
something that is emotionally neutral (2 trials; 1 low-intensity, 1
high-intensity). </br> Reappraisal: thinking about an image in a way
that reduces its negative meaning (2 trials; 1 low-intensity, 1
high-intensity). </br> </br> **2. Practice phase.** Participants will
complete 8 practice trials: 4 trials with have pre-determined regulation
strategy (one trial for each strategy at each intensity level) and 4
trials with participants freely choosing which strategy to use. </br>
</br> **3. Experimental phase.** Participants will view 30 images for
500ms and choose one of two regulation strategies to implement for each
image. They will then view each image for 5,000ms while implementing the
chosen strategy. The same 30 images that were used in the original study
by Sheppes et al (2011) will be used
...
**Exclusion criteria.** Participants who fail the attention check at the
end of the study will be excluded from analyses (participants will be
presented with 9 images: 5 that appeared as part of the experiment and 4
that did not, and asked to select the images that were part of the
experiment.)
...
Link to experiment:
<https://web.stanford.edu/~kpetrova/cgi-bin/experiment.html>

I didn't manage to get access to the full IAPS database by this
assignment's submission deadline, so, as of Sunday 10/24, the experiment
was set up with the same 4 images across all training, practice, and
experimental trials. I expect to get access to the full IAPS during the
week of 10/25, at which stage I will update my experimental setup as
follows.
...
pilot_a_tidy <- pilot_a |>
  ...
  group_by(subj_ID, stimulus) |>
  summarise(
            percent_reappraisal = ((sum(reappraisal)/(sum(distraction)+sum(reappraisal)))*100),
            percent_distraction = ((sum(distraction)/(sum(distraction)+sum(reappraisal)))*100)) |>
  mutate(intensity = factor(stimulus, levels=c(1, 2), labels=c("Low", "High"))) |>
  select(-stimulus)
...
model_pilot_a <- lmer(percent_reappraisal ~ intensity + (1|subj_ID), pilot_a_tidy)
